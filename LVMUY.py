...
if __name__ == "__main__":
    ...
    keyword = "#lvmh"
    lvmh1 = pd.read_csv(keyword + ".csv", sep=";", error_bad_lines=False)
    lvmh2 = pd.read_csv(keyword + "2.csv", sep=";", error_bad_lines=False)    
    t_df= pd.concat([lvmh1, lvmh2])
    ...
    trace_price = go.Candlestick(x=df['Date'], open=df['Open'], high=df['High'],
                    low=df['Low'], close=df['Close'], name='Stock: LVMUY')
    ...
    fig.update_layout(title_text="Sentiment Analysis (Vader) & Stock Price : LVMH (ticker: LVMUY)",
